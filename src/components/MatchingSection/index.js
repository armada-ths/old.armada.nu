...
class MatchingSection extends React.Component {
    ...
    submit() {
        if (
            ...
            this.postData('https://ais.armada.nu/api/matching/', {
                industries: {
                    answer: this.state.industries.map(i => i.id),
                    weight: this.state.weights[1],
                },
                values: {
                    answer: this.state.values.map(i => i.id),
                    weight: this.state.weights[0],
                },
                employments: {
                    answer: this.state.employments.map(i => i.id),
                    weight: this.state.weights[3],
                },
                locations: {
                    answer: this.state.locations.map(i => i.id),
                    weight: this.state.weights[4],
                },
                competences: {
                    answer: this.state.competences.map(i => i.id),
                    weight: this.state.weights[2],
                },
                ...
                <div className='wrapper'>
                    <div className='matching-question-card'>
                        <MatchingQuestion
                            ...
                            prevDisabled={this.state.optionIndex === 0}
                            prevClick={this.prevOption}
                            nextClick={this.nextOption}
                            answers={option.answers}
                            handleChange={this.handleChange(
                                this.state.optionIndex
                            )}
                            preSelected={this.getResult(this.state.optionIndex)}
                            onWeightChange={this.onWeightChange}
                            index={this.state.optionIndex}
                            weight={this.state.weights[this.state.optionIndex]}
                        />
                    </div>
                </div>
            </div>
                    ...
                    <div className='wrapper' style={background}>
                        <div className={dynamicclass}>
                            ...
                            <span className='stars'>
                                {this.createStars(result.similarity)}
                            </span>
    ...
    presentMatches() {
        ...
        if (this.state.match_result) {
            for (
                let i = 0;
                i < this.state.match_result.similarities.total.length;
                i++
            ) {
                listitems.push(
                    this.createCard(
                        this.state.match_result.similarities.total[i],
                        i === 0
                    )
                )
            }
    ...
    presentMoreMatches() {
        ...
        for (var cat in similarities) {
            ...
            similarities[cat].forEach(result => {
                ...
                listitems.push(this.createCard(result, false))
            })
        }
    ...
    handleChange = index => {
        ...
        return function (value) {
            if (index === 0) {
                bindedthis.setState({ values: value })
            }
            if (index === 1) {
                bindedthis.setState({ industries: value })
            }
            if (index === 2) {
                bindedthis.setState({ competences: value })
            }
            if (index === 3) {
                bindedthis.setState({ employments: value })
            }
            if (index === 4) {
                bindedthis.setState({ locations: value })
            }
        }
    }
    ...
    renderQuestions() {
        return this.state.started === true && this.state.isLoading === false ? (
            this.carousel(this.state.currentOption)
        ) : (
        ...
        return (
            <div>
                <div className='questions'>
                    ...
                    <br />
                    <br />
                    {!this.state.match_result ? null : this.state.show_more ? (
                        <div>
