...
const KnightecSpeakers = () =>{
    ...
    const createCards = (start, end) => {
        return (speakers.filter((i, index) => (index < end && index >= start)).map((info, index) => <ContactCard name={info.name} key={index} workTitle={info.workTitle} education={info.education} videoUrl={info.videoUrl} linkedInUrl={info.linkedInUrl} profileImageUrl={info.profileImageUrl} zoomUrl={info.zoomUrl} description={info.description}  />))
      }
